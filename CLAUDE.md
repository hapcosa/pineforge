...
# Estructura de Alertas (OBLIGATORIA)

> pineforge es el repo para **crear y adaptar** los Pine del ecosistema. Todo
> script que vaya a producción DEBE emitir el JSON canónico de abajo. Es
> requisito de compatibilidad con la API de trading: el mensaje viaja
> `alert() → webhook TradingView → pending_signals → ejecución`. Fuente de
> verdad del contrato: `dashboard/backend/webhooks_api.py` (`_parse_payload` +
> `_parse_risk_block`).
...
```json
{
  "side": "buy",              // buy|sell|long|short|bull|bear   (OBLIGATORIO)
  ...
  "risk": {                   // opcional — plan de risk management
    ...
    "trailing":  {"activation_pct": 2.0, "callback_pct": 0.8},
    "breakeven": {"trigger_pct": 1.0,  "offset_pct": 0.1}
  }
}
```
...
1. **`side` + `symbol` son lo único obligatorio.** Falta uno → la señal se descarta.
2. **`symbol` limpio.** Usar `syminfo.basecurrency + "USDT"` (o input explícito).
   NUNCA `{{ticker}}`: el parser quita `EXCHANGE:` y `/ - _`, pero **no el `.`**,
   así que `BTCUSDT.P` llega sucio y no matchea contra el catálogo `cryptos`.
3. **`alert.freq_once_per_bar_close`** salvo estrategia explícitamente intrabar.
   Debe coincidir con cómo se backtestea en KryptoLab (señal al cierre de vela).
   La condición de entrada ya debe estar gateada por `barstate.isconfirmed`.
4. **`risk.tp`: máximo 3 entradas, `qty_pct` 0..100.** La suma de `qty_pct` no
   debe pasar 100 (footgun: el parser corta a 3 pero NO valida la suma; el
   trailing nativo cierra el remanente si la suma es <100). Poner `maxval=100`
   por TP en los inputs.
5. **El `strategy_token` va en la URL del webhook**, no en el body — se
   configura en la alerta de TradingView, no en el script.
...
```pine
// Grupo de inputs del plan de RM (exponer aunque el script no los dibuje)
G_RM = "⑧ Risk plan (alertas)"
alertSym  = input.symbol("", "Símbolo alerta (vacío = basecurrency+USDT)", group=G_RM)
...
Las `alertcondition()` de texto (para alertas manuales en TV) pueden convivir
con el `alert()` JSON; **no** son sustituto del webhook.

> Implementación de referencia (los 7 scripts ya migrados): `Osciladores/budai_pulse.pine`,
